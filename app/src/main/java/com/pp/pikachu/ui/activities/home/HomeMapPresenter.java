...
public class HomeMapPresenter
    ...
    OnItemClickListener, OnSaveFavoriteListener, OnRemoveFavoriteListener {
  ...
  public void processMarker(GoogleMap googleMap) {
    ...
    for (int i = 0; i < this.airportsList.size(); i++) {
      ...
      if (!airportsList.get(i).getName().equals(homeMapFragment.sharedPreferenceManager.getFavoriteAirport())) {
        googleMap.addMarker(new MarkerOptions().position(airport).title(this.airportsList.get(i).getCity()));
      } else {
  ...
  @Override public void onConnected(@Nullable Bundle bundle) {
    if (ActivityCompat.checkSelfPermission(homeMapFragment.getActivity(), Manifest.permission.ACCESS_FINE_LOCATION)
        != PackageManager.PERMISSION_GRANTED
        && ActivityCompat.checkSelfPermission(homeMapFragment.getActivity(), Manifest.permission.ACCESS_COARSE_LOCATION)
        != PackageManager.PERMISSION_GRANTED) {
      ActivityCompat.requestPermissions(homeMapFragment.getActivity(),
    ...
    if (location == null) {
      ...
      sortLocations();
      Toast.makeText(homeMapFragment.activity, homeMapFragment.activity.getString(R.string.sorted), Toast.LENGTH_LONG)
          .show();
    }
  }
  ...
  public void sortLocations() {
    ...
    Collections.sort(airportsList, (airports, t1) -> {
      ...
      return (int) (location1.distanceTo(myCurrentLocation) - location2.distanceTo(myCurrentLocation));
    });
  }
}
